...
int main(int argc, char **argv) {
    ...
    int mySizeY = viewPlaneResolutionY/size;
    ...
    for(int y = 0; y < mySizeY; y++) {
        const auto color = viewPlane.computePixel(scene, x, y+mySizeY*rank, numOfSamples);
    ...
    if (rank == 0) {
       ...
       for(int x = 0; x<srcSizeX; x++)
	    ...
	    finalImage.set(x,y+srcSizeY*rank,srcPiece.get(x,y));
} }
